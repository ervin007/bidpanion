...
# ─────────────────────────────────────────────────────────────────────
# Trim notes vs. the 22-field version:
#   CUT  list_of_documents          — duplicate of required_documents
#   CUT  forms_esubmission          — rarely populated, conceptually misplaced
#   CUT  headcount_staffing         — team-shape facts now live in
#                                     personnel_profiles; absolute volumes in
#                                     contract_volume
#   MERGE subcontractor_identification (Legal) into subcontracting_consortia
#         (Scope) — single home for policy + required declaration forms
# 22 → 18 fields. Reference-projects-of-personnel vs. company-references
# split is preserved because your routing rules explicitly distinguish them.
# ─────────────────────────────────────────────────────────────────────
...
- WHAT must be delivered / HOW the service is performed
    → Scope & Requirements
- Numeric size of the contract (total PT, hours, EUR, ceiling values)
    → Scope & Requirements → Contract Volume
- Physical location, on-site requirement, remote share
    → Scope & Requirements → Place of Performance
- Subcontractors and consortia (BOTH the rules AND the required forms)
    → Scope & Requirements → Subcontracting & Consortia
- PERSONNEL skills / certifications / seniority / team-shape per role
...
FIELDS = [
    ...
    # ── Project Description ──────────────────────────────────────────
    {
        ...
        "instruction": (
            ...
            "(PT, hours, EUR) belong in Contract Volume. Per-role staffing "
            "and team shape belong in Personnel Profiles. Award weights "
            "belong in Award Criteria. "
    ...
    # ── Scope & Requirements ─────────────────────────────────────────
    {
        ...
        "instruction": (
            ...
            "  stated as part of the service (e.g. SAP PO/IS, UiPath, "
            "  ServiceNow). "
            ...
            "personnel qualifications and team shape (→ Personnel Profiles), "
            "bidder reference projects (→ Company Referrals), award scoring "
            "(→ Award Criteria). "
    ...
    },
    {
        ...
        "instruction": (
            "The numeric SIZE of the contract: total budget, total "
            "Personentage (PT), total hours, EUR amount, ceiling values. "
            "Per-Los and per-Leistungskategorie breakdowns are wanted here "
            "when the source provides them as a volumes table "
            ...
            "Do NOT include: per-role team-shape constraints such as "
            "'mindestens 2 Senior Consultants verfügbar' — those go to "
            "Personnel Profiles."
        ),
        "extraction_mode": "narrative",
        "mandatory": False,
    ...
    },
    {
        ...
        "queries": [
            ...
            "Eignungsleihe Drittunternehmen Verpflichtungserklärung",
            "Haftung gesamtschuldnerisch bei Nachunternehmern",
            "Liste Identifikation Nachunternehmer Formblatt einzureichen",
            "Genehmigung Auftraggeber Wechsel Nachunternehmer DSGVO",
        ],
        "instruction": (
            "Everything related to using subcontractors and forming consortia, "
            "covering BOTH the policy/rules AND the required declaration "
            "forms in one place. "
            "Capture: "
            "- whether subcontractors / Bietergemeinschaften are admitted at "
            "  all and under what conditions, "
            "- approval and notification rules (genehmigungspflichtiger "
            "  Wechsel, vorherige Anzeige), "
            "- liability arrangements (gesamtschuldnerische Haftung), "
            "- DSGVO / data-processing obligations passed through to "
            "  subcontractors, "
            "- Eignungsleihe (third-party reliance) rules, "
            "- restrictions on switching subcontractors during the contract, "
            "- the specific declaration forms the bidder must submit "
            "  (Verpflichtungserklärung von Drittunternehmen, Erklärung der "
            "  Bietergemeinschaft, Liste der Nachunternehmer, "
            "  Subunternehmererklärung). "
            "Output: detailed narrative for the policy, then a short bullet "
            "list of the required forms at the end. "
            "The form names also appear in 'Offer Submission Documents' — "
            "that's expected; here we capture them with their substantive "
            "context."
        ),
        "extraction_mode": "narrative",
        "mandatory": False,
    ...
    },
    {
        ...
        "instruction": (
            ...
            "Do NOT include subcontractor-related declarations "
            "(→ Scope & Requirements → Subcontracting & Consortia)."
        ),
        "extraction_mode": "list",
        "mandatory": False,
    ...
    # ── Technical & Professional Ability ─────────────────────────────
    {
        ...
        "queries": [
            ...
            "Anzahl Mitarbeiter pro Rolle FTE verfügbar Team",
        ],
        "instruction": (
            "Requirements for the people the bidder will deploy. This field "
            "covers BOTH the qualitative profile per role AND the team-shape "
            "constraints (how many of each role) — both are typically stated "
            "together at role level in the source. "
            ...
            "  >10 Jahre = 10 Punkte'), "
            "- TEAM-SHAPE constraints stated alongside role requirements: "
            "  required FTE per role, minimum number of people per role, "
            "  availability windows (e.g. 'mindestens 2 Senior Consultants "
            "  während der gesamten Vertragslaufzeit verfügbar', 'innerhalb "
            "  von 5 Werktagen abrufbar'). "
            "Format: nested bullets, Los → Rolle → Anforderungen "
            "(Qualifikationen, dann Team-Shape). "
            "Do NOT include: total contract volumes / PT / EUR (→ Contract "
            "Volume — that field is for contract-level totals, not per-role "
            "team shape); reference projects required of those personnel "
            "(→ Reference Projects); company-level references "
